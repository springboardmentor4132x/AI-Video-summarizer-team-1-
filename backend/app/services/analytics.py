...
def _summary_text(summary: Summary | None) -> str:
    if summary is None:
        return ""
    return " ".join(filter(None, [summary.short_summary, summary.detailed_summary]))
...
def _video_score(video: Video) -> tuple[float, list[ScoreComponent]]:
    ...
    moments = video.key_moments or []
    transcript_words = _word_count(transcript.text if transcript else None)
    summary_words = _word_count(_summary_text(summary))
    status_lower = _value(video.status).lower()
    processing = 100 if status_lower == "completed" else 55 if status_lower in PROCESSING_STATUSES else 20 if status_lower in PENDING_STATUSES else 0
    transcript_score = 100 if transcript and _value(transcript.status).upper() == "COMPLETED" else 50 if transcript else 0
    transcript_score = min(transcript_score, round(transcript_words / 5)) if transcript_words else 0
    summary_score = 100 if summary and _value(summary.status).upper() == "COMPLETED" else 45 if summary else 0
    summary_score = min(summary_score, 100 if summary_words >= 40 else round(summary_words / 40 * 100)) if summary_words else 0
    moment_score = min(100, len(moments) * 20) if status_lower == "completed" else 0
...
def build_dashboard(db: Session, user_id: int | None = None, date_from: date | None = None, date_to: date | None = None) -> AnalyticsDashboard:
    ...
    status_rows = db.execute(select(Video.status, func.count(Video.id)).where(*video_conditions).group_by(Video.status)).all()
    status_counts = {_value(status).lower(): int(count) for status, count in status_rows}
    ...
    processing_rows = db.execute(
        select(func.date(Video.uploaded_at), func.count(Video.id))
        .where(*video_conditions)
        .where(func.lower(Video.status).in_((*PROCESSING_STATUSES, "completed", "failed")))
    ...
    recent_video_rows = db.execute(
        ...
        .options(
            selectinload(Video.transcript).selectinload(Transcript.summary),
            selectinload(Video.key_moments)
        )
    ...
    summary_rows = db.scalars(
        select(Summary)
        .join(Transcript, Transcript.id == Summary.transcript_id)
        .join(Video, Video.id == Transcript.video_id)
        .where(*summary_conditions)
    ).all()
    summary_texts = [_summary_text(summary) for summary in summary_rows]
    summary_word_counts = [_word_count(text) for text in summary_texts]
    total_summaries = len(summary_rows)
    completed_summaries = sum(_value(summary.status).upper() == "COMPLETED" for summary in summary_rows)
    
    videos_with_summaries = len(set(db.scalars(
        select(Transcript.video_id)
        .join(Summary, Summary.transcript_id == Transcript.id)
        .join(Video, Video.id == Transcript.video_id)
        .where(*summary_conditions)
    ).all()))

    summary_characters = sum(len(text) for text in summary_texts)
    recent_summary_rows = db.execute(
        select(Summary, Video.filename, Video.id)
        .join(Transcript, Transcript.id == Summary.transcript_id)
        .join(Video, Video.id == Transcript.video_id)
        .where(*summary_conditions)
    ...
    moment_duration = sum(max(0.0, float(moment.end_time - moment.start_time)) for moment in moment_rows)
    importance_scores = [float(moment.importance_score) for moment in moment_rows]
    recent_moment_rows = db.execute(
        select(KeyMoment, Video.filename)
        .join(Video, Video.id == KeyMoment.video_id)
        .where(*moment_conditions)
        .order_by(KeyMoment.importance_score.desc(), KeyMoment.created_at.desc())
        .limit(20)
    ).all()
    ...
    grouped_moments = [CountItem(label=video_names.get(vid, str(vid)), count=count) for vid, count in moments_by_video.most_common()]

    total_duration = round(moment_duration)
    average_duration = round(moment_duration / video_count, 2) if video_count else 0.0
    ...
    content_activity = [
        ContentActivityItem(date=day, **counts)
    ...
    importance_distribution = [
        ImportanceDistributionItem(label=label, count=len(scores), average_importance=round(sum(scores) / len(scores), 2) if scores else 0)
    ...
    for video, owner_name in recent_video_rows:
        ...
        video_intelligence.append(RecentVideo(
            id=str(video.id), filename=video.filename, owner_name=owner_name or "Unknown",
            status=_value(video.status), uploaded_at=video.uploaded_at,
            duration_seconds=int(max([m.end_time for m in moments], default=0)) if moments else None,
    ...
    for video, owner_name in recent_video_rows[:20]:
        transcript = video.transcript
        summary = transcript.summary if transcript else None
        moment_count = len(video.key_moments or [])
        ...
        if _value(video.status).lower() == "completed" and video.uploaded_at and _in_date_range(video.uploaded_at, date_from, date_to):
            activity.append(RecentActivity(type="VIDEO_COMPLETED", timestamp=video.uploaded_at, video_id=str(video.id), video_name=video.filename, description=f"{video.filename} finished processing"))
        if transcript and _value(transcript.status).upper() == "COMPLETED" and transcript.created_at and _in_date_range(transcript.created_at, date_from, date_to):
        ...
        if summary and _value(summary.status).upper() == "COMPLETED" and summary.created_at and _in_date_range(summary.created_at, date_from, date_to):
        ...
        recent_moments = [moment for moment in (video.key_moments or []) if _in_date_range(moment.created_at, date_from, date_to)]
        if recent_moments:
            max_moment_ts = max((moment.created_at for moment in recent_moments if moment.created_at), default=video.uploaded_at)
            activity.append(RecentActivity(type="KEY_MOMENTS_DETECTED", timestamp=max_moment_ts, video_id=str(video.id), video_name=video.filename, description=f"{len(recent_moments)} key moments detected in {video.filename}"))
    activity.sort(key=lambda item: item.timestamp, reverse=True)

    transcript_insights = TranscriptInsights(
        total_transcripts=total_transcripts,
        total_words=total_transcript_words,
        average_words_per_transcript=round(total_transcript_words / total_transcripts, 2) if total_transcripts else 0,
        longest_transcript_words=max(transcript_word_counts, default=0),
        shortest_transcript_words=min(transcript_word_counts, default=0),
        total_characters=transcript_characters,
        average_characters=round(transcript_characters / total_transcripts, 2) if total_transcripts else 0,
    )
    ...
    component_averages = []
    if score_components and score_components[0]:
        for index, template in enumerate(score_components[0]):
    ...
    content_insights = ContentInsights(
        total_transcripts=total_transcripts,
        total_transcript_characters=transcript_characters,
        average_transcript_characters=round(transcript_characters / total_transcripts, 2) if total_transcripts else 0,
        total_summary_characters=summary_characters,
        top_keywords=keyword_insights,
        total_transcript_words=total_transcript_words,
        average_words_per_video=round(total_transcript_words / video_count, 2) if video_count else 0,
        key_moment_density=round(total_key_moments / total_transcript_words, 4) if total_transcript_words else 0,
    )
    summary_insights = SummaryReports(
        total_summaries=total_summaries,
        completed_summaries=completed_summaries,
        videos_with_summaries=videos_with_summaries,
        generation_rate_percentage=round(completed_summaries / total_transcripts * 100, 2) if total_transcripts else 0,
        total_summary_characters=summary_characters,
        recent_activity=[RecentSummary(id=str(summary.id), video_id=str(vid_id), filename=fname, status=_value(summary.status), created_at=summary.created_at) for summary, fname, vid_id in recent_summary_rows],
        total_summary_words=sum(summary_word_counts),
        average_summary_words=round(sum(summary_word_counts) / total_summaries, 2) if total_summaries else 0,
        longest_summary_words=max(summary_word_counts, default=0),
        shortest_summary_words=min(summary_word_counts, default=0),
    )
    key_moment_insights = KeyMomentAnalytics(
        total_key_moments=total_key_moments,
        videos_with_key_moments=videos_with_key_moments,
        average_per_video=round(total_key_moments / video_count, 2) if video_count else 0,
        total_duration_seconds=round(moment_duration, 2),
        average_duration_seconds=round(moment_duration / total_key_moments, 2) if total_key_moments else 0,
        recent_activity=[RecentKeyMoment(id=str(moment.id), video_id=str(moment.video_id), filename=filename, title=moment.title, topic=moment.topic, importance_score=moment.importance_score, start_time=moment.start_time, end_time=moment.end_time) for moment, filename in recent_moment_rows],
        average_importance=round(sum(importance_scores) / total_key_moments, 2) if total_key_moments else 0,
        highest_importance=max(importance_scores, default=0),
        by_video=grouped_moments,
    )
    ...
    return AnalyticsDashboard(
        ...
        summary_reports=summary_insights,
        content_insights=content_insights,
        key_moment_analytics=key_moment_insights,
        usage=UsageAnalytics(total_users=int(total_users), users_by_role=_count_items(role_rows), upload_activity=video_activity, processing_activity=processing_activity),
        processing_insights=ProcessingInsights(
            videos=_pipeline_stage(video_count, video_count),
            transcripts=_pipeline_stage(total_transcripts, video_count),
            summaries=_pipeline_stage(completed_summaries, total_transcripts),
            key_moments=_pipeline_stage(videos_with_key_moments, completed_videos),
        ),
        transcript_insights=transcript_insights,
        keyword_insights=keyword_insights,
        content_insights_v2=content_insights,
        summary_insights=summary_insights,
        key_moment_insights=key_moment_insights,
        ...
        compression_insights=CompressionInsights(
            average_transcript_words=round(average_transcript_words, 2),
            average_summary_words=round(average_summary_words, 2),
            compression_ratio=round(average_summary_words / average_transcript_words, 4) if average_transcript_words else 0.0,
        ),
