...
def create_user_and_video(email: str, video_id_override=None):
    db = TestingSessionLocal()
    # These tests exercise ownership after role authorization has succeeded.
    user = User(name="User", email=email, password="hash", role="Content Creator")
    db.add(user)
    db.flush()
    ...
    return SimpleNamespace(id=user_id, role="Content Creator"), SimpleNamespace(id=video_id, transcript_id=transcript_id)
